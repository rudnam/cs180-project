...
class ClusterPredictor():
  def __init__(self, transfer_data_path, transfer_df_income_path, best_k, pca_reduced=True):
    ...
    self.col_rank_dict = copy.copy(transfer_data['col_rank_dicts'])
    ...
    default_center = [np.mean(df_income[col]) for col in self.std_cols]
    self.default_center = pd.DataFrame([default_center], columns=self.std_cols)
    self.col_stds = pd.DataFrame([[np.std(df_income[col]) for col in self.num_cols]], columns=self.num_cols)
    self.col_means = pd.DataFrame([[np.mean(df_income[col]) for col in self.num_cols]], columns=self.num_cols)
  
  def get_cluster_center(self, cluster_i):
    cluster_members = self.df_income[self.model.labels_ == cluster_i]
    cluster_members = cluster_members[self.std_cols]
    return [np.mean(cluster_members[col]) for col in self.std_cols]
  ...
  def get_df_sample(self, website_data):
    ...
    for feat in self.cat_features:
      ...
      for uniq in self.df_income[feat].unique():
        uniq0 = uniq
        uniq = underscore_formatter(uniq)
    ...
    df_final = pd.DataFrame([sample_z_scores], columns=std_cols_sample)
    df_model = copy.deepcopy(self.df_income[self.std_cols])
  ...
  def predict(self, website_data):
    ...
    if self.pca_reduced==False: return self.model.predict(df_sample)
    min_index = 0
    ...
    resulting_cluster = min_index + 1
    print(f"Website sample belongs to cluster {resulting_cluster}.")
